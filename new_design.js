  ...
  function init() {
    ...
    if (cardOuter && !document.querySelector('.plaan-tooltip-card')) {
      ...
      tooltip.innerHTML =
        ...
        'Нажмите кнопку «Восстановить пароль».<br>' +
        'Введите почту, которую указывали при записи на обучение.<br>' +
        'Подтвердите отправку — мы пришлём письмо с доступом.<br>' +
        'Откройте письмо и перейдите по ссылке.<br>' +
        'В личном кабинете зайдите в раздел «Профиль».<br>' +
        'Установите новый пароль — и вы снова в игре \uD83D\uDE80' +
        '</div></div>' +
